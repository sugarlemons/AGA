...
if __name__ == '__main__':
    ...
    # matrix of agc and acc
    agc_arr = [[0] * 4 for i in range(4)]
    acc_arr = [[0] * 4 for i in range(4)]
    dnodes_degree = 0
    pedges_degree = 0
    mask_nodes_degree = 0
    subgraph_degree = 0
    ...
    for i in range(4):
        for j in range(4):
            # six combined augmentations
            if args.aug == 'sd':
                subgraph_degree = degree1 = i * space + 0.1
                dnodes_degree = degree2 = j * space
            elif args.aug == 'sp':
                subgraph_degree = degree1 = i * space + 0.1
                pedges_degree = degree2 = j * space
            elif args.aug == 'sm':
                subgraph_degree = degree1 = i * space + 0.1
                mask_nodes_degree = degree2 = j * space
            elif args.aug == 'dp':
                dnodes_degree = degree1 = i * space
                pedges_degree = degree2 = j * space
            elif args.aug == 'dm':
                dnodes_degree = degree1 = i * space
                mask_nodes_degree = degree2 = j * space
            else:
                pedges_degree = degree1 = i * space
                mask_nodes_degree = degree2 = j * space
            print('degree1:{}, degree2:{}'.format(degree1, degree2))
            dataset = TUDataset(path, name=DS, aug=args.aug, dnodes_degree=dnodes_degree, pedges_degree=pedges_degree,
                                subgraph_degree=subgraph_degree, mask_nodes_degree=mask_nodes_degree).shuffle()
            ...
            agc_arr[i][j] = agc
            acc_arr[i][j] = np.mean(accuracies['test'])
            ...
            with open(file_path, 'a+') as f:
                ...
                f.write('augment degree:{},{}\n'.format(degree1, degree2))
                f.write('agc:{}\n'.format(agc))
                f.write('\n')
            accuracies = {'val': [], 'test': []}

    # draw the heat map of agc and acc
    index = []
    columns = []
    fig, axes = plt.subplots(nrows=1, ncols=2)
    fig.subplots_adjust(wspace=0.4)
    if args.aug == 'dp':
        index = ['0', '0.05', '0.10', '0.15']
        columns = ['0', '0.05', '0.10', '0.15']
    elif args.aug == 'sd':
        index = ['0', '0.05', '0.10', '0.15']
        columns = ['0.10', '0.15', '0.20', '0.25']
    elif args.aug == 'dm':
        index = ['0', '0.05', '0.10', '0.15']
        columns = ['0', '0.05', '0.10', '0.15']
    elif args.aug == 'sp':
        ...
        index = ['0', '0.05', '0.10', '0.15']
        columns = ['0.10', '0.15', '0.20', '0.25']
    df_agc = pd.DataFrame(agc_arr, index=index, columns=columns)
    df_acc = pd.DataFrame(acc_arr, index=index, columns=columns)
    sns.heatmap(df_agc, ax=axes[0], cmap='Blues')
    sns.heatmap(df_acc, ax=axes[1], cmap='Blues')
