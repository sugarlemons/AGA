...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
                 ...
                 cleaned=False, aug=None, dnodes_degree=0, pedges_degree=0,
                 subgraph_degree=0, mask_nodes_degree=0):
        self.name = name
        self.cleaned = cleaned
        self.dnodes_degree = dnodes_degree
        self.pedges_degree = pedges_degree
        self.subgraph_degree = subgraph_degree
        self.mask_nodes_degree = mask_nodes_degree
    ...
    def get(self, idx):
        ...
        if self.aug == 'dnodes':
            data_aug = drop_nodes(deepcopy(data), self.dnodes_degree)
        elif self.aug == 'pedges':
            data_aug = permute_edges(deepcopy(data), self.pedges_degree)
        elif self.aug == 'subgraph':
            data_aug = subgraph(deepcopy(data), self.subgraph_degree)
        elif self.aug == 'mask_nodes':
            data_aug = mask_nodes(deepcopy(data), self.mask_nodes_degree)
        elif self.aug == 'dp':
            data_aug = drop_nodes(deepcopy(data), self.dnodes_degree)
            data_aug = permute_edges(deepcopy(data_aug), self.pedges_degree)
        elif self.aug == 'sd':
            data_aug = subgraph(deepcopy(data), self.subgraph_degree)
            data_aug = drop_nodes(deepcopy(data_aug), self.dnodes_degree)
        elif self.aug == 'dm':
            data_aug = drop_nodes(deepcopy(data), self.dnodes_degree)
            data_aug = mask_nodes(deepcopy(data_aug), self.mask_nodes_degree)
        elif self.aug == 'sp':
            data_aug = subgraph(deepcopy(data), self.subgraph_degree)
            data_aug = permute_edges(deepcopy(data_aug), self.pedges_degree)
        elif self.aug == 'pm':
            ...
            data_aug = drop_nodes(deepcopy(data), self.dnodes_degree)
            data_aug = permute_edges(deepcopy(data_aug), self.pedges_degree)
            data_aug = subgraph(deepcopy(data_aug), self.subgraph_degree)
            data_aug = mask_nodes(deepcopy(data_aug), self.mask_nodes_degree)
        elif self.aug == 'none':
            """
...
# degree: augmentation strength  [0,1], default 0.1
def drop_nodes(data, degree):
    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
    drop_num = int(node_num * degree)
...
# degree: augmentation strength  [0,1], default 0.1
def permute_edges(data, degree):
    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
    permute_num = int(edge_num * degree)
...
# degree: augmentation strength  [0,1], default 0.2
def subgraph(data, degree):

    node_num, _ = data.x.size()
    _, edge_num = data.edge_index.size()
    sub_num = int(node_num * degree)
...
# degree: augmentation strength  [0,1], default 0.1
def mask_nodes(data, degree):
    node_num, feat_dim = data.x.size()
    mask_num = int(node_num * degree)
